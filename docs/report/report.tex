...
($W$) and response read ($R$), so it schedules $R$ just after $W$: at runtime
this means that $R_i$ is executed in the cycle following $W_i$, but the cache
response has a certain latency which makes $R_i$ stall (and $W_{i+1}$ too, by
consequence), therefore the cache never receives multiple requests in
consecutive cycles and never reaches the steady state.
...
and $R$, so that at runtime \texttt{LATENCY} requests are sent to cache before
stalling.
...
scheduled. This situation not only breaks the proposed solution, but it also
makes the \ac{ii} of the loop accessing the cache to increase. 
In this case it is convenient to set \texttt{LATENCY} to 1.

To actually solve this problem it is probably needed to have more control on the
scheduler behaviour, which is not possible with
\emph{Vitis\texttrademark~HLS~2021.1}.
