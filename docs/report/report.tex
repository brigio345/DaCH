...
This Subsection is mainly referred to
...
\bigskip
The pipelining can be performed at instruction level, whithin a loop or a
...
at a time (where $f$ is the unroll factor).
...
includes an \texttt{hls::stream} object, exploiting the template specialization
mechanism.
...
\begin{figure}[!htb]
	...
	\caption{\emph{Multi-level cache} architecture.}
	\label{fig:l1_arch}
\end{figure}
...
\begin{itemize}
	\item \texttt{get}: it calls \texttt{get\_line} function and then
