...
to multiple concurrent accesses to the same cache memory, allowing to
efficiently unroll application loops.

\subsection{Architecture}
The \emph{Multi-ports cache} is characterized by multiple access ports
...
\emph{Multi-process} internals architecture is generated.
...
guaranteees all the dependencies on cache data structures, and the resulting
...
Each call to \texttt{get\_line} (which is in turn called by \texttt{get}) and
\texttt{set} is associated with a specific port by means of a member variable
