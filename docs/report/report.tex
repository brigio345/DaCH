...
kernels implementing different algorithms, to check both the performance gain
and the resource usage of different cache configurations.

The results have been compared with the performance lower bound (algorithm
accesses data directly from external \ac{dram}), with the higher bound
(algorithm accesses data from local \acp{bram}) and with the Ma's cache.

The different algorithms has been chosen for their specific memory access
patterns.
...
\ac{hw} Emulation has been used only for determining the delay of the \ac{axi}
interface (around 4 clock cycles), then all the simulations have been performed
by using Co-Simulation, with \ac{axi} latency set to 3, to match the results by
\ac{hw} emulation as much as possible.

\bigskip
A summary of the simulation environment configuration shown in
Table~\ref{tab:sim_config}.
...
To make a fair comparison with \emph{Ma's cache}, it should be synthesized and
simulated in the exact same environment.
Since it was designed for the \emph{Vivado\texttrademark~HLS 2016.2}, it is not
getting good performance when synthesized (after some adaptations) in
\emph{Vitis\texttrademark~HLS~2021.1}.

The most comparable results are therefore the ones reported in the
\citetitle{liang} paper and in the \citetitle{liangthesis} thesis, both by Ma.

\section{Matrix multiplication}
The standard row-by-column \emph{Matrix multiplication} algorithm
(Algorithm~\ref{alg:matmul}) include two memory access patterns: $A$ and $C$
matrices are accessed by rows, while $B$ matrix is accessed by columns.
...
\subsection{\texttt{get} latency exploration}
\begin{table}[!htb]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{\emph{Matrix multiplication} setup $A$.}
	\label{tab:matmul_setup_a}
\end{table}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=.8\textwidth]{matmul_latency_exploration}
	\caption{Execution time with respect to \texttt{get} latency of
	\emph{Matrix multiplication}.}
	\label{fig:matmul_latency_exploration}
\end{figure}
...
\begin{table}[!htb]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{\emph{Matrix multiplication} setup $B$.}
	\label{tab:matmul_setup_b}
\end{table}
...
\begin{itemize}
	...
	\item Sets: 1.
	\item Ways: 2.
	\item Words per line: 16.
	\item \texttt{get} latency: 2.
\end{itemize}
